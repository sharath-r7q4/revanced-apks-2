...
patch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		eval java -jar *cli*.jar  $p$b --keystore-entry-password=$KEYSTORE_PASS --keystore-password=$KEYSTORE_PASS --signer=$KEYSTORE_ALIAS --keystore-entry-alias=$KEYSTORE_ALIAS $m$opt --out=./release/$1-$2.apk$excludePatches$includePatches$ks $pu$force $a./download/$1.apk
  		unset version
		unset lock_version
